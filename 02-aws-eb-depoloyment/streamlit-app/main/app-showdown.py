...
st.markdown(
    "<h1 style='text-align: center;'>CREDIT SCORING API</h1>", unsafe_allow_html=True
)
st.markdown(
    "**This Streamlit app utilizes a predictive model to calculate and display the probability of default for individuals or entities.\
...
data["seniority"] = st.number_input(
    "Job Seniority (Years)", min_value=0, step=1, value=2
)

data["income"] = st.number_input("Amount of Income ($)", min_value=0, step=1, value=200)

data["assets"] = st.number_input(
    "Amount of Assets ($)", min_value=0.0, step=1.0, value=200.0, format="%.2f"
)


data["time"] = st.number_input(
    "Time of Requested Loan (Months)", min_value=1, step=1, value=1
)
...
data["id"] = st.number_input("Enter the customer's ID", min_value=1, step=1, value=1)
...
if st.button("Send request"):
    ...
    dict_final = preprocess_scaled_customer_dataframe(
        scaled_df=df_scaled, customer_id=cust_id
    )

    # Send the request using the dictionary and write the result in the Streamlit app
    st.write(
        send_request(
            customer_dict=dict_final,
            url="http://prediction-api-env.eba-kusyz2y7.eu-north-1.elasticbeanstalk.com.",
        )
    )
